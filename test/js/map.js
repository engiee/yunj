var flylineMap = function(option) {
	var defaults = {
		...
		curvature : 0.0005, // 实际指焦点到准线的距离，你可以抽象成曲率，这里模拟扔物体的抛物线，因此是开口向下的
			...
			if(params.areaBackgroundColorGradient){
				...
				for(var infoId = 0 ; infoId < data.length; infoId ++){
					...
					if(d.properties.id == info.id){
						svg.append("text")
						.attr("id","area-info-content-"+i)
						.attr("fill",params.areaContentFontColor)
						.attr("x",xy[0])
						.attr("y",xy[1])
						.text(info.content);
						d3.select("#area-info-content-"+i).attr("x",function(){
	...
	var tooltip = function(mouseCoords, content) {
		if (params.showContent) {
			...
			if (tooltips.length < params.showContentNum) {
				...
				id = nvtooltipid;
				tooltips.push(tip);
				var tipHtml = '<div id="' + nvtooltipid + '" style="pointer-events:none;position:absolute;width:' + params.contentWidth + 'px;height:auto;min-height:50px;line-height:60px;background:'
						+ params.contentBackgroudColor + ';border:2px solid ' + params.contentBorderColor + ';border-radius:4px; text-align:center;color:' + params.contentFontColor + '">';
				...
				tipHtml += '<div style="position:absolute;left:30px;overflow:hidden;width:0;height:0;border-width:10px;border-style:solid dashed dashed dashed;bottom:-20px;border-color:'
						+ params.contentBorderColor + ' transparent transparent transparent; "></div>';
	...
	var showAreaTootip = function(mouseCoords, content) {
		if (params.showAreaTootip) {
			...
			tip.id = nvtooltipid;
			id = nvtooltipid;
			var tipHtml = '<div id="' + nvtooltipid + '" style="position:absolute;width:' + params.areaTipWidth + 'px;height:auto;min-height:50px;line-height:60px;background:'
					+ params.areaTipBackgroudColor + ';border:2px solid ' + params.areaTipBorderColor + ';border-radius:4px; text-align:center;color:' + params.areaTipFontColor + '">';
			tipHtml += '<div style="width:100%" id="' + nvtooltipid + 'Content"></div>';
			tipHtml += '</div>';
			tipJquery = $(tipHtml);
			tipJquery.appendTo('body');
